...
import ceka.simulation.LabelingStrategy;
...
public class SingleQualLabelingStrategy extends LabelingStrategy {
	...
	/**
	 * @see LabelingStrategy# labelingDataset(ceka.core.Dataset)
	 */
	@Override
	public void labelDataset(Dataset dataset, MockWorker mockWorker) {
		...
		for (int k = 0; k < numCategory; k++) {
			int correct = (int)(mockWorker.getSingleQuality() * exampleLists.get(k).size()); //正确的实例数
			int incorrect = exampleLists.get(k).size() - correct; //错误的实例数
	...
	public void labelDatasetMy(Dataset dataset, MockWorker mockWorker,List instIndex) {
		...
		for (int i = 0; i < dataset.getExampleSize(); i++) {
			Example example = dataset.getExampleByIndex(i);
			int trueCate = example.getTrueLabel().getValue();
			if (instIndex.contains((Integer)i)) {
				exampleLists.get(trueCate).add(example);
			}
		}
